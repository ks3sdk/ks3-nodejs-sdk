 ...
 *
 */
 function listObjectsV2 (params, cb) {
 ...
 * }
 */
 function getVersioning(params, cb) {
	if(typeof params === 'function') {
		cb = params;
		params = {};
	}
	var date = (new Date()).toUTCString();
	var resource = '/?versioning'
	var bucketName = params.Bucket || this.bucketName || '';
	if(!bucketName) {
		throw new Error('require the bucketName');
	}
 ...
 * }
 */
 function putVersioning(params, cb) {
	if(typeof params === 'function') {
		cb = params;
		params = {};
	}
	...
	if(!~['Enabled', 'Suspended'].indexOf(status)) {
	...
	var body = '<VersioningConfiguration xmlns="http://s3.amazonaws.com/doc/2006-03-01/"><Status>' + status + '</Status></VersioningConfiguration>';
	// 计算content-md5   request body里data的128位md5 digest，再用base64编码。
	var md5 = crypto.createHash('md5').update(body).digest('base64');
	req.headers['Content-MD5'] = md5;
	var curAuth = auth.generateAuth(this.ak, this.sk, req);
