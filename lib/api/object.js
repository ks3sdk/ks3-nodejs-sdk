...
function get(params, cb) {
	...
	var resource = params.VersionId ? '/' + util.encodeKey(params.Key) + '?versionId=' + params.VersionId : '/' + util.encodeKey(params.Key);
	var bucketName = params.Bucket || this.bucketName || '';
...
function put(params, cb, headers) {
	var bucketName = params.Bucket || this.bucketName || '';
	...
	var req = {
		...
		type: params.isNoContent ?  '': require('mime').lookup(Key),
		date: util.getDate(),
		uri: config.protocol + '://' + bucketName + '.' + config.baseUrl + resource,
		resource: '/' + bucketName + resource,
		headers: headers || {}
	...
	if (tagging && util.verifyTagging(tagging)) {
		var attr_Tagging = 'x-' + config.prefix + '-tagging';
		req.headers[attr_Tagging] = getTaggingConfigurationString(params.Tagging);
	}
...
function getTaggingConfigurationString(tagging) {
	var result = '';
	if(Array.isArray(tagging)) {
		result = tagging.map(function(item) {
			return item.value ? encodeURIComponent(item.key) + '=' + encodeURIComponent(item.value) : encodeURIComponent(item.key);
		}).join('&');
	}
...
function getTaggingConfigurationXML(tagging) {
	var result = '<Tagging><TagSet>';
	
	for(var i = 0, l = tagging.length; i < l; i++) {
		result += tagging[i].value ? '<Tag><Key>' + tagging[i].key + '</Key><Value>' + tagging[i].value + '</Value></Tag>' : '<Tag><Key>' + tagging[i].key + '</Key></Tag>';
	}
...
function putTagging(params, cb) {
	...
	if (!Tagging) {
		throw new Error('require the Tagging');
	}
	if (!util.verifyTagging(Tagging)) {
		throw new Error('the illegal Tagging');
	}
	var bucketName = params.Bucket || this.bucketName || '';
	if(!bucketName) {
		throw new Error('require the bucket name');
	...
	var key = util.encodeKey(params.Key);
	var resource = params.VersionId ? '/' + key + '?tagging&versionId=' + params.VersionId : '/' + key + '?tagging';
	...
	var authStr = auth.generateAuth(this.ak, this.sk, req);
	var body = getTaggingConfigurationXML(Tagging);
	request(req, body, authStr, cb);
}
...
module.exports = {
	...
	putTagging: putTagging,
	getTagging: getTagging,
	delTagging: delTagging
}
