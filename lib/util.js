 ...
 * 验证Tagging 
 */
function verifyTagging(tagging) {
	var flag = true;
	var reg = /^[a-zA-Z0-9\-\x20\+\=\.\_\:\/]*$/;
	if (Array.isArray(tagging) && tagging.length > 0) {
		for (var i = 0, l = tagging.length; i < l; i++) {
			var key = tagging[i].key;
			var value = tagging[i].value || '';
			if (typeof key !== 'string' || typeof value !== 'string') {
				flag = false;
				return false;
			}
			if (!key || !reg.test(key) || !reg.test(value)) {
				flag = false;
				return false;
			}
			...
			if (key.startsWith('kss:') || key.startsWith('ksc:')) {
				flag = false;
				return false;
			}
		}
